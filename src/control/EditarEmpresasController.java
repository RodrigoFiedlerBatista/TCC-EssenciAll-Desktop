...
public class EditarEmpresasController implements Initializable {
    ...
    private void checaUsuario() {
        ...
        if (Usuario.getUsuarioLogado() >= 0) {
            System.out.println("FOI usuario");
            ...
            for (int i = 0; i < usuarios.size(); i++) {
                for (int j = 0; j < revende.size(); j++) {
                    if (usuarios.get(Usuario.getUsuarioLogado()).getId_usuario() == revende.get(j).getId_usuario()) {
                        id_revende = revende.get(j).getId_revende();
    ...
    void cadastrar(ActionEvent event) {
        ...
        if (checaMarcas()) {
            ...
            revende.setId_revende(id_revende);
            revendeDAO.editaRevende(revende);
            tcc.fechaTela();
            tcc.iniciaStage("ContaRevendedor.fxml");
            alertas.empresasEditadas();
        }
    }
